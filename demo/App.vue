...
const updateDepthInput = (value: string) => { depthInput.value = value; scheduleDepthCommit() }
const scheduleDepthCommit = () => {
  if (depthApplyTimer !== undefined) window.clearTimeout(depthApplyTimer)
  depthApplyTimer = window.setTimeout(() => { depthApplyTimer = undefined; commitDepth() }, INPUT_APPLY_DELAY_MS)
}
const commitDepth = () => {
  if (depthApplyTimer !== undefined) { window.clearTimeout(depthApplyTimer); depthApplyTimer = undefined }
...
const updateLumaKeyThresholdInput = (value: string) => { lumaKeyThresholdInput.value = value; scheduleLumaKeyThresholdCommit() }
const scheduleLumaKeyThresholdCommit = () => {
  ...
  lumaKeyThresholdApplyTimer = window.setTimeout(() => { lumaKeyThresholdApplyTimer = undefined; commitLumaKeyThreshold() }, INPUT_APPLY_DELAY_MS)
}
const commitLumaKeyThreshold = () => {
  if (lumaKeyThresholdApplyTimer !== undefined) { window.clearTimeout(lumaKeyThresholdApplyTimer); lumaKeyThresholdApplyTimer = undefined }
  const nextThreshold = Number(lumaKeyThresholdInput.value)
  if (!Number.isFinite(nextThreshold)) { lumaKeyThresholdInput.value = String(lumaKeyThreshold.value); return }
...
const destroyInstance = () => { if (instance.value) { instance.value.destroy(); instance.value = null } }
const createInstance = async () => {
  ...
  try {
    ...
    const config = {
      ...
      ...(sourceChoice.value === SourceChoice.Usermedia ? {} : { source: new URL(sourceUrl.value, document.baseURI).href }),
    } as const
    ...
    nextInstance.on(NyxEvent.Ready, () => { isUpdating.value = false; setStatus('Live', `${sourceChoice.value} is mounted with ${theme.value} at depth ${depth.value.toFixed(2)}.`, NyxTheme.Success) })
    ...
    nextInstance.ready.catch((error: NyxErrorEvent['error']) => {
      if (instance.value === nextInstance) { isUpdating.value = false; setStatus('Needs attention', error.message, NyxTheme.Danger) }
    })
...
const chooseSource = async (choice: SourceChoice) => { sourceChoice.value = choice; if (choice === SourceChoice.Image) sourceUrl.value = imageUrl; if (choice === SourceChoice.Video) sourceUrl.value = videoUrl; await nextTick(); await createInstance() }
const startWebcam = async () => { sourceChoice.value = SourceChoice.Usermedia; await nextTick(); await createInstance() }
const applySource = async () => { await createInstance() }
const copyExample = async () => {
  try { if (!navigator.clipboard) throw new Error('Clipboard access is unavailable in this browser.'); await navigator.clipboard.writeText(quickstartCode.value); copyLabel.value = 'Copied' }
  catch (error) { copyLabel.value = 'Copy failed'; setStatus('Needs attention', error instanceof Error ? error.message : 'Clipboard access was denied.', NyxTheme.Warning) }
  window.setTimeout(() => { copyLabel.value = 'Copy example' }, 1600)
}
const labelNyxControls = () => {
  document.getElementById('source-url')?.setAttribute('aria-describedby', 'url-help')
  ...
  if (control) { control.id = 'theme-select-control'; control.setAttribute('aria-label', 'Particle theme') }
}
onMounted(async () => { await nextTick(); labelNyxControls(); void createInstance() })
...
<template>
  <main class="site-shell">
    <header class="topbar"><a class="wordmark" href="#top" aria-label="NyxFission home"><span class="wordmark-mark">N</span><span>nyx<span class="wordmark-muted">fission</span></span></a><nav aria-label="Page sections"><a href="#playground">Playground</a><a href="#reference">Reference</a><a href="https://github.com/NyxKit/nyx-fission">GitHub</a></nav></header>
